...
public:
	...
	EL_TSimpleString(const T* ptr, EL_SIZE len=(EL_SIZE)-1, EL_UINT32 hash32=0) : m_ptr(ptr ? ptr : nullptr), m_len(len), m_hash32(hash32)
	{		
	}
...
{return m_ptr ? ECtoWC(GetPtr(), GetLen(), bufPtr, bufSize) : 0;}
...
{return wcscmp(GetPtr() + wcslen(GetPtr()) - test.GetLen(), test.GetPtr()) == 0;}
